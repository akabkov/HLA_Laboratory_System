...
**HLA Laboratory System** — настольное приложение для лаборатории HLA-диагностики, автоматизирующее импорт результатов с анализатора, ведение файловой базы пациентов, поиск по архиву исследований и подготовку выходных документов для практической работы врачей.
...
- запускается в ограниченном режиме, если файловая база или PostgreSQL временно недоступны; при недоступной файловой базе главное окно открывается, но импорт и боковой проводник временно недоступны, а при недоступном PostgreSQL блокируется импорт исследований.
...
- запуск с ограничениями при недоступности внешних ресурсов; при недоступной файловой базе приложение открывается, но импорт и боковой проводник временно недоступны, а при недоступном PostgreSQL блокируется импорт исследований;
...
- сохранять снимок состояния с возможностью отката критичных файловых операций;
...
автоматическим сохранением в PDF, одной из частых причин ошибок является то,
что Microsoft Word не доверяет папке, из которой открыт исходный документ.

Чтобы Word корректно выполнял преобразование DOCX -> PDF, добавьте рабочую
...
После этого Word будет автоматически доверять всем файлам из этой папки.

## Адаптация под другую лабораторию

В актуальной версии проекта логика, зависящая от учреждения, опирается на два
внутренних кода: `f_clinic` и `s_clinic`. В большинстве случаев для адаптации под другую
лабораторию их можно не переименовывать: достаточно заменить отображаемые
...
7. Проверить логику применения учреждения в
   ...
   «Сформировать заключение» теперь работают как read-only индикатор текущей
   настройки из окна «Настройки», а пресеты чекбоксов `И. о.` и `Врач`
...
9. Выполнить запуск из исходников и проверочный прогон ключевых сценариев:
   импорт CSV, поиск по файловой базе, формирование документов в форматах DOCX
   и PDF, а также подключение к PostgreSQL.
...
### Как быстро поднять PostgreSQL 18 для проекта
...
Если файловая база или PostgreSQL временно недоступны, приложение всё равно запускается в ограниченном режиме и показывает главное окно. При недоступной файловой базе временно недоступны импорт и боковой проводник. При недоступном PostgreSQL в текущей версии не выполняется импорт исследований: перед импортом приложение останавливает операцию и предлагает проверить параметры подключения к БД.
...
быстрый поиск был готов сразу после запуска. Для крупного корня, особенно
...
в ближайшем обновлении рядом с кнопкой проводника есть кнопка
`Обновить`, чтобы перестройку индекса можно было запускать вручную в удобный
момент, не включая её обязательно на старте приложения.
...
- **многопользовательская**: один ПК `A` хранит живую файловую базу и PostgreSQL, а остальные ПК подключаются к ним по сети.
...
- **ПК `A`** хранит живую файловую базу, например `D:\HLA_Laboratory_System`, и на нём же работает PostgreSQL;
- **ПК `B`, `C`, ...** запускают тот же дистрибутив приложения, используют сетевой путь к живой базе на `A` и подключаются к PostgreSQL на `A`.
...
2. Расшарить эту папку по SMB, например как:
...
9. Если на `A` используется импорт Word-заключений с автоматическим сохранением в PDF, настроить Microsoft Word по разделу «Важно для DOCX -> PDF».

### Что настроить на каждом дополнительном ПК `B`, `C`, ...

1. Установить ту же сборку приложения, что и на `A`.
2. Проверить, что с клиента открывается живая сетевая папка на `A`, например:
...
3. В приложении открыть `Настройки` и задать:
   - `Путь к файловой базе`: UNC-путь к живой базе на `A`, например `\\A\HLA_Laboratory_System`
...
6. Если на клиенте выполняется импорт Word-заключений и DOCX -> PDF, настроить Microsoft Word и доверенные расположения на этом клиенте тоже.
...
- Все клиенты должны работать с **живой** файловой базой на `A`, а не с её зеркальной копией.
...
Сценарий автоматического резервного копирования PostgreSQL создаёт локальный дамп в составе файловой базы
`D:\HLA_Laboratory_System`, поэтому при включённой зеркализации этот дамп
также может попадать в сетевое зеркало вместе с остальными файлами базы.
...
Если приложение устанавливается на боевой ПК целиком в каталог вида
...
Именно на такой путь сейчас ориентирован установочный PowerShell-скрипт
регистрации задачи Планировщика Windows.

Эти скрипты не обязательны для локального запуска приложения из исходников, но
