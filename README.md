...
![Главное окно приложения](assets/main_window.png)
...
- запускается в ограниченном режиме, если сетевой диск или PostgreSQL временно недоступны.
...
- новый формат: `Иванов_Алексей.Андреевич_01.01.1980_m`
- с новой фамилией: `Иванова(Петрова)_Мария_12.03.1992_f`
- с двойной фамилией: `Сидоров-Петров_Андрей.Сергеевич_07.05.1988_m`
...
- запуск с ограничениями при недоступности внешних ресурсов;
...
```powershell
pyinstaller --noconfirm --clean HLA_Laboratory_System.spec
```
...
- в папках исследований — управляемые файлы (`1_класс.xlsx`, `2_класс.xlsx`, `идентификация.jpg`, `скрининг.jpg` в соответствующих сценариях).
...
CREATE TABLE
    tests (
        ...
        UNIQUE (patient_id, organ_id, test_date, hla_class),
...
CREATE INDEX idx_tests_organ_patient_date
    ON tests (organ_id, id) INCLUDE (patient_id, test_date);

CREATE INDEX idx_antibodies_test_specificity_raw_value
    ON antibodies (test_id, gene) INCLUDE (allele_group, raw_value);
```
...
Если файловая база доступна, а PostgreSQL временно нет, приложение все равно сможет запуститься в ограниченном режиме, но функции синхронизации положительных результатов с БД будут недоступны.
...
Проект также использует сторонние компоненты с отдельными условиями лицензирования, включая Qt for Python, SQLAlchemy, pandas, openpyxl, python-docx, psycopg2-binary и другие зависимости, перечисленные в комплекте лицензионных notices.
