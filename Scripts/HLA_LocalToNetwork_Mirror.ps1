...
# ------------------------------------------------------------
$StateDir = 'C:\ProgramData\HLA_Mirror'
...
# ------------------------------------------------------------
$mutexName = 'Global\HLA_LocalToNetwork_Mirror'
$createdNew = $false
...
function Get-RobocopyArguments {
    ...
    if ($robocopyCapabilities.Dcopy) {
        ...
        Write-RobocopyCompatibilityWarning -Key 'DCOPY' -Message 'Current robocopy does not support /DCOPY. Directory metadata copy options will be skipped.'
    }

    if ($robocopyCapabilities.It) {
        # Also copy files when only attributes differ
        $args += '/IT'
    }
    else {
        Write-RobocopyCompatibilityWarning -Key 'IT' -Message 'Current robocopy does not support /IT. Attribute-only changes may be skipped.'
    }
    ...
    if ($IoRate) {
        if ($robocopyCapabilities.IoRate) {
            ...
            if ($Threshold) {
                if ($robocopyCapabilities.Threshold) {
                    $args += "/THRESHOLD:$Threshold"
                }
                else {
                    Write-RobocopyCompatibilityWarning -Key 'THRESHOLD' -Message 'Current robocopy does not support /THRESHOLD. The threshold option will be skipped.'
                }
            }
        }
        else {
    ...
    if ($UseCompression) {
        if ($robocopyCapabilities.Compress) {
            ...
            Write-RobocopyCompatibilityWarning -Key 'COMPRESS' -Message 'Current robocopy does not support /COMPRESS. Compression will be skipped.'
        }
    }
...
Register-ObjectEvent -InputObject $watcher -EventName Changed -SourceIdentifier 'HLA_MIRROR_Changed' | Out-Null
Register-ObjectEvent -InputObject $watcher -EventName Created -SourceIdentifier 'HLA_MIRROR_Created' | Out-Null
Register-ObjectEvent -InputObject $watcher -EventName Deleted -SourceIdentifier 'HLA_MIRROR_Deleted' | Out-Null
Register-ObjectEvent -InputObject $watcher -EventName Renamed -SourceIdentifier 'HLA_MIRROR_Renamed' | Out-Null
Register-ObjectEvent -InputObject $watcher -EventName Error   -SourceIdentifier 'HLA_MIRROR_Error'   | Out-Null
...
try {
    while ($true) {
        ...
        if ($evt) {
            do {
                if ($evt.SourceIdentifier -eq 'HLA_MIRROR_Error') {
