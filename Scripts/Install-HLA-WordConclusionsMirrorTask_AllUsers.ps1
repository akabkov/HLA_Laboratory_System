...
[CmdletBinding()]
param(
    [string]$TaskName = 'HLA Word Conclusions Mirror',
    [string]$SyncScriptPath = 'C:\Program Files\HLA_Laboratory_System\_internal\Scripts\HLA_WordConclusions_LocalToNetwork_Mirror.ps1',
...
if ($TaskName -match '[\\/]') {
    throw "TaskName must not contain path separators. Use a plain task name, for example 'HLA Word Conclusions Mirror'."
}
...
if ([string]::IsNullOrWhiteSpace($Source)) {
    throw "Source must not be empty. Specify the local source path via -Source."
}
