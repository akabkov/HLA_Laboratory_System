...
# --- Вставка и обновление основных сущностей исследования ---
...
def insert_test(
    ...
    if pra is None:
        raise RuntimeError(
            "Внутренняя ошибка: insert_test вызван без обязательного значения PRA."
        )

    result = conn.execute(
        text("""--sql
            INSERT INTO
                tests (patient_id, test_date, hla_class, pra)
            VALUES
                (:patient_id, :test_date, :hla_class, :pra) ON CONFLICT (patient_id, test_date, hla_class)
            DO
            UPDATE
            SET
                pra = EXCLUDED.pra
...
# --- Поиск и обновление уже существующих пациентов ---
...
def get_patient_recipient_code_by_code(
    conn,
    patient_code: str,
) -> tuple[bool, int | None]:
    row = (
    ...
    return True, row["recipient_code"]


def get_patient_code_by_recipient_code(
    conn,
    recipient_code: int,
    organ: str,
) -> str | None:
    result = conn.execute(
        text("""--sql
            ...
            WHERE
                p.recipient_code = :recipient_code
                AND o.title = :organ
        """),
        {
            "recipient_code": recipient_code,
...
def get_patient_codes_by_recipient_code(
    conn,
    recipient_code: int,
) -> list[str]:
    result = conn.execute(
...
def update_patient_recipient_code_by_code(
    conn,
    patient_code: str,
    recipient_code: int | None,
) -> int | None:
    ...
    result = conn.execute(
        text("""--sql
            ...
            SET
                ...
                recipient_code = :recipient_code
            WHERE
                patient_code = :current_patient_code
            RETURNING
                id
        """),
        {
...
# --- Удаление тестов и очистка пациентов-сирот ---
...
def delete_test_by_patient_code(
    ...
    *,
    ...
    result = conn.execute(
        text("""--sql
            DELETE FROM tests t USING patients p,
            organs o
            WHERE
                t.patient_id = p.id
                AND p.organ_id = o.id
