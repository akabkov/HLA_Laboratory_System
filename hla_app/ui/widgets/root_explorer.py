...
# ctypes-обвязка Windows Shell/OLE для внешнего drag&drop.
...
class RootExplorerWidget(QWidget):
    ...
    def _bind_model_signals(self) -> None:
        ...
        self.model.rowsInserted.connect(self._on_model_structure_changed)
        self.model.rowsRemoved.connect(self._on_model_structure_changed)
        self.model.directoryLoaded.connect(self._on_model_directory_loaded)
        self.model.modelReset.connect(self._on_model_structure_changed)
    ...
    def _ensure_tree_root_scope(self) -> QModelIndex:
        if getattr(self, "model", None) is None or self.tree.model() is None:
            return QModelIndex()

        current_root_index = self.tree.rootIndex()
        if current_root_index.isValid() and self._paths_match(
            self._path_from_index(current_root_index),
            self._root_dir,
        ):
            return current_root_index

        root_index = self._exact_tree_index_for_path(self._root_dir)
        if not root_index.isValid():
            source_root_index = self.model.setRootPath(str(self._root_dir))
            root_index = self._map_from_source(source_root_index)
            if not self._paths_match(
                self._path_from_index(root_index),
                self._root_dir,
            ):
                return QModelIndex()
    ...
    def _on_model_structure_changed(self, *_args) -> None:
        if not self._ensure_tree_root_scope().isValid():
            return
    ...
    def _on_model_directory_loaded(self, path: str) -> None:
        root_index = self._ensure_tree_root_scope()
        if not root_index.isValid():
            return
    ...
    def _start_windows_shell_drag(self, paths: list[Path]) -> bool:
        ...
        try:
            ...
            if data_object_address is not None:
