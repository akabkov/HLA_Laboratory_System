...
from hla_app.config.managed_files import MANAGED_STUDY_FILE_NAMES
...
class RootExplorerWidget(QWidget):
    def __init__(
        ...
        *,
        ...
        # Важно: левая панель не должна диктовать ширину всему QSplitter
        # только потому, что в дереве есть длинные имена файлов/папок.
        ...
        self.search_edit.setToolTip(
            "Поиск по имени файла или папки в текущей выделенной папке.\n"
        ...
        self._pending_navigation_timer.setInterval(
            _SEARCH_NAVIGATION_RETRY_INTERVAL_MS
        )
        ...
        self._pending_navigation_timer.timeout.connect(
            self._retry_pending_navigation
        )
        ...
        # Не позволяем дереву раздувать весь интерфейс по ширине
        # из-за длинных имён элементов.
        ...
        # Колонка дерева может быть шире viewport,
        # поэтому при длинных именах появится собственный
        # горизонтальный скролл у QTreeView.
    ...
    def minimumSizeHint(self) -> QSize:
        # Минимум по ширине должен быть стабильным и не зависеть
        # от содержимого QFileSystemModel / QTreeView.
        return QSize(EXPLORER_MIN_WIDTH, EXPLORER_MIN_WIDTH * 2)

    def sizeHint(self) -> QSize:
        # Предпочтительная ширина панели проводника.
    ...
    def _update_tree_column_width(self) -> None:
        ...
        # Пересчитываем ширину первой колонки по содержимому,
        # чтобы при длинных именах и глубокой вложенности
        # у дерева мог появиться свой горизонтальный скролл.
    ...
    def _update_search_results_height(self) -> None:
        ...
        for row in range(count):
            row_height = self.search_results.sizeHintForRow(row)
            if row_height <= 0:
                row_height = self.search_results.fontMetrics().height() + 8
        ...
        max_height = max(
            self.search_results.fontMetrics().height() + frame + 8,
    ...
    def _navigate_to_path(self, path: Path) -> None:
        ...
        def apply_focus_and_scroll():
            ...
            # Снимаем визуальный фокус со списка результатов поиска
            self.search_results.clearSelection()
            self.search_results.clearFocus()

            # Делаем найденный элемент текущим в дереве
            self.tree.setCurrentIndex(index)

            # Прокручиваем так, чтобы элемент оказался в видимой области,
            # лучше сразу ближе к центру viewport
            self.tree.scrollTo(index, QAbstractItemView.PositionAtCenter)

            # Передаём фокус самому дереву
            self.tree.setFocus(Qt.OtherFocusReason)

        # Первый вызов — сразу
        apply_focus_and_scroll()

        # Второй — после того как Qt успеет дорисовать/перелэйаутить дерево
        QTimer.singleShot(0, apply_focus_and_scroll)

        if path.is_dir():
            QTimer.singleShot(0, lambda p=Path(path): self._expand_search_match_if_needed(p))
    ...
    def _build_search_placeholder(self) -> str:
        ...
        if base_dir_resolved == root_resolved:
            # Если root_dir приложения НЕ является корнем диска,
            # показываем именно имя папки файловой базы, а не имя тома.
            if root_resolved.parent != root_resolved:
                folder_name = (root_resolved.name or "").strip()
                if folder_name:
                    return f"Поиск {folder_name}"
                return f"Поиск {str(root_resolved)}"

            # Только если root_dir сам является корнем файловой системы,
            # имеет смысл показывать имя/метку тома.
            ...
            if display_name and drive:
                if drive.casefold() in display_name.casefold():
                    return f"Поиск {display_name}"
                return f"Поиск {display_name} ({drive})"

            if display_name:
                return f"Поиск {display_name}"

            if drive:
                return f"Поиск {drive}"

            return f"Поиск {root_resolved.name or str(root_resolved)}"

        folder_name = (base_dir_resolved.name or "").strip()
        if folder_name:
            return f"Поиск {folder_name}"

        return f"Поиск {str(base_dir_resolved)}"
    ...
    def _update_search_placeholder(self) -> None:
        ...
        self.search_edit.setToolTip(
            "Поиск по имени файла или папки в текущей директории поиска.\n"
            "Если выбран файл, поиск выполняется в его родительской папке.\n\n"
            f"Текущая директория поиска:\n{base_dir}"
        )
    ...
    def _is_dir_visible_in_tree(self, dir_path: Path) -> bool:
        ...
        while current != root_resolved:
            ...
            # Для папок первого уровня относительно root_dir
            # проверка развернутости не нужна:
            # они и так видимы в дереве.
    ...
    def _clear_search_if_active(self) -> None:
        if not self.search_edit.text().strip():
            return
    ...
    def _apply_search_now(self) -> None:
        ...
        try:
            ...
            # --- Новый быстрый режим: поиск по локальному индексу ---
            ...
            # --- Резервный режим: старый поиск по дереву/сети ---
            self._clear_search_results()
    ...
    def _apply_pending_inline_commit(self) -> None:
        ...
        # Для новой папки пользователь мог оставить имя по умолчанию.
        # В этом случае пароль всё равно уже подтверждён, а переименование не нужно.
    ...
    def _show_context_menu(self, pos) -> None:
        ...
        # Если клик был по пустому месту, считаем, что контекстное меню открыто
        # для ROOT_DIR, а не для текущего выделения.
        ...
        if chosen is act_open:
            self._run_after_menu_close(lambda p=path: self._open_path(p))
            return

        if chosen is act_reveal:
            self._run_after_menu_close(lambda p=path: self._reveal_in_system(p))
            return

        if chosen is act_refresh:
            self._run_after_menu_close(self.refresh)
            return

        if chosen is act_add_files and act_add_files is not None:
            self._run_after_menu_close(lambda td=target_dir: self._add_files(td))
            return

        if chosen is act_new_folder and act_new_folder is not None:
            self._run_after_menu_close(lambda td=target_dir: self._create_folder(td))
            return

        if chosen is act_rename and act_rename is not None:
            self._run_after_menu_close(lambda p=path: self._rename_path(p))
            return

        if chosen is act_delete and act_delete is not None:
            self._run_after_menu_close(lambda p=path: self._delete_path(p))
    ...
    def _is_protected_path(
        ...
        *,
        ...
        if path.name in MANAGED_STUDY_FILE_NAMES:
            return True, (
                "Операции с файлами 1_класс.xlsx, 2_класс.xlsx, "
                "скрининг.jpg и идентификация.jpg разрешены только "
    ...
    def _create_folder(self, parent_dir: Path) -> None:
        ...
        parent_index = self.model.index(str(parent_dir))
        if not parent_index.isValid():
            QMessageBox.warning(
                self,
                "Не удалось создать папку",
                f"Не удалось получить индекс папки:\n{parent_dir}",
            )
            return
        ...
        tree_parent_index = self._tree_index_for_path(parent_dir)
        if tree_parent_index.isValid():
            self.tree.expand(tree_parent_index)

        new_index = self.model.mkdir(parent_index, folder_name)
