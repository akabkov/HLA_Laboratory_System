...
ORGANS = ["ГЕМАТОЛОГИЯ", "ЛЁГКОЕ", "ПЕЧЕНЬ", "ПОЧКА", "СЕРДЦЕ"]
...
# Раскладка клавиатуры приложения по умолчанию.
INPUT_LANGUAGE_ID = 0x0419
...
def get_builtin_db_name_for_clinic(clinic: str | None) -> str:
...
def get_default_db_name_for_clinic(clinic: str | None) -> str:
    ...
    return get_builtin_db_name_for_clinic(clinic)


def get_opposite_clinic_code(clinic: str | None) -> str:
    clinic_code = clinic or DEFAULT_CLINIC
    ...
    raise ValueError(
        "Неизвестное учреждение для противоположной клиники: "
        f"{clinic_code!r}. Ожидалось 'f_clinic' или 's_clinic'."
    )


def get_builtin_opposite_db_name_for_clinic(clinic: str | None) -> str:
    return get_builtin_db_name_for_clinic(get_opposite_clinic_code(clinic))
