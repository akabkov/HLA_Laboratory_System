...
# ..._дд.мм.гггг или ..._дд.мм.гггг__2
...
def _raise_fs_locked_runtime_error(action: str, target, exc: Exception) -> None:
    raise RuntimeError(
        ...
        "(например, Excel, просмотр JPG/изображений), и повторите операцию."
    ) from exc
...
def rename_source_files_for_patient(
    ...
    try:
        ...
        # 3. Откат: всё, что уже переименовали в финальные имена, возвращаем во временные
        for new_path, tmp_path in reversed(renamed_to_final):
            if new_path.exists():
                _rename_path_with_retry(new_path, tmp_path)

        # 4. Возвращаем временные обратно в исходные имена
...
def save_conclusion_jpg(
    ...
    *,
    ...
    dst_name = conclusion_file_name(negative_screening=negative_screening)
