...
# ФИО: первые 2 символа ОБЯЗАТЕЛЬНО буквы
RU_FIO_RE = re.compile(r"^[А-Яа-яЁё]{2}[А-Яа-яЁё-]*$")

# --- Правила нормализации для поиска совпадений пациентов ---
...
# --- Сравнение и нормализация строк для поиска совпадений ---
...
def is_valid_ru_fio(s: str) -> bool:
    s = (s or "").strip()

    if not s:
        return False

    if not RU_FIO_RE.match(s):
        return False
...
# --- Разбор и форматирование дат пользовательского ввода ---
...
# --- Сборка имен папок пациента и идентификатора patient_code ---
...
# --- Нормализация регистра и фамилий с дефисами ---
...
def cap_hyphenated_fio_part(s: str) -> str:
    s = (s or "").strip()
