...
def _write_optional_clinic(
    ...
    if text not in _ALLOWED_CLINIC:
        raise ValueError("clinic должен быть 'mnpc', 'rnpc' или пустым значением.")
...
def load_effective_app_preferences() -> AppPreferences:
    ...
    return AppPreferences(
        ...
        clinic=(user.clinic if user.clinic is not None else DEFAULT_CLINIC),
...
def save_clinic_preference(clinic: str | None) -> None:
    """Сохраняет только учреждение по умолчанию."""
    settings = _settings()
    _write_optional_clinic(settings, _KEY_CLINIC, clinic)
    settings.sync()
