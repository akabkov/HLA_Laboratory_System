...
def do_import(
    *, root_dir: Path, patient: PatientData, inp: ImportInput
    ...
    has_csv_selection = bool(selected_csvs)
    has_conclusion_selection = inp.conclusion_file_path is not None
    ...
    if not has_csv_selection and not has_conclusion_selection:
        return ImportResult(
    ...
    try:
        for hla_class, csv_path in selected_csvs.items():
            ...
            if source_dir is None:
                raise RuntimeError(
                    "Внутренняя ошибка импорта: не подготовлен каталог source_files."
                )
        ...
        if inp.conclusion_file_path is not None:
            ...
            save_conclusion_file(
                ...
                negative_screening=not has_csv_selection,
            )
        ...
        if has_csv_selection:
            with get_engine().begin() as conn:
                ...
                for hla_class, pack in parsed_selected.items():
                    ...
                    if organ_id is None or patient_id is None:
                        raise RuntimeError(
                            "Внутренняя ошибка импорта: не удалось подготовить пациента для записи результатов."
                        )

                    pra_value = _to_int(pra)
                    if pra_value is None:
                        raise RuntimeError(
                            f"В выбранном CSV отсутствует обязательное значение PRA для Class {hla_class}."
                        )
                    ...
                    # Для существующего теста полностью заменяем набор антител текущим CSV.
                    conn.execute(
                        text("""--sql
