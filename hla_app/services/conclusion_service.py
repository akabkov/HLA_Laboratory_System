...
# --- Нормализация подписей и ФИО для заключения ---
...
# --- Подготовка словаря данных по классу для DOCX-шаблона ---
...
# --- Генерация рекомендуемого имени итогового файла заключения ---


def _effective_conclusion_clinic_for_filename() -> str:
    ...
    raise ValueError(
        "Не удалось определить учреждение для имени файла: "
        "clinic должна быть 'mnpc' или 'rnpc'."
    )
...
def suggest_conclusion_filename(
    ...
    clinic = _effective_conclusion_clinic_for_filename()

    if clinic == "mnpc":
        return f"{num_register}_Ab_{safe_last}_{init}.docx"

    return f"{num_register}_{safe_last}_{init}.docx"
